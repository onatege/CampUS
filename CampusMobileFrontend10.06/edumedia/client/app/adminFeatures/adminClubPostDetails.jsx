...
function AdminClubPostDetails() {
    ...
    useEffect(() => {
        if (isDeleteSuccess) {
            router.push({ pathname: "admin/clubManagement", params: { userId: user?.id } })
        }
    }, [isDeleteSuccess])
            ...
            <View style={styles.container}>
                <View style={styles.header}>
                    <View style={{ width: '100%', height: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <TextBody color={'black'}>{clubPost?.data?.data?.clubName}</TextBody>
                ...
                <View style={styles.content}>
                    <TextBody color={'black'}>{clubPost?.data?.data.content}</TextBody>
                </View>
                {
