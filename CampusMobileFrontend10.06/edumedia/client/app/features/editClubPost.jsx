...
function EditClubPost() {
    const { user } = useSelector(state => state.user);
    const { clubPostId, clubId } = useLocalSearchParams(); // clubPostId'yi URL parametresinden alın
    const { data: clubPost, isLoading: isPostLoading } = useGenericQueryHook(`/api/ClubPost/GetClubPostById?clubPostId=${clubPostId}`, `${clubPostId}ClubPost`);
    ...
    useEffect(() => {
        if (clubPost) {
            setContent(clubPost?.data?.data?.content);
            setImages(clubPost?.data?.data?.images);
        }
    }, [clubPost]);
    ...
    const handleUpdatePost = () => {
        const postData = {
            endpoint: `/api/ClubPost/UpdateClubPost?clubPostId=${clubPostId}`,
            key: [`${clubId}Club`, `${clubPostId}ClubPost` ],
            data: {
