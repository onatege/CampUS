...
function AdminClubPostItem({ item, isMember, clubId }) {
    const router = useRouter();
    const [activePage, setActivePage] = useState(0);

    const handleRedirectProfile = (id, e) => {
        e.preventDefault();
        router.push({ pathname: "adminFeatures/adminClubDetails", params: { clubId: clubId } });
    };

    const handleRedirectClubPostDetail = () => {
        router.push({pathname:"adminFeatures/adminClubPostDetails", params:{clubPostId:item?.clubPostId, isMember: isMember, clubId:clubId}})
    }

    return (
        <View style={styles.container}>
            <TouchableOpacity onPress={(e) => handleRedirectProfile(item.id, e)} style={styles.profileImageContainer}>
                <Image
            ...
            <View style={styles.contentContainer}>
                <TouchableOpacity onPress={ () => handleRedirectClubPostDetail() } style={styles.header}>
                ...
                <TouchableOpacity onPress={ () => handleRedirectClubPostDetail() }>
...
export default AdminClubPostItem;
